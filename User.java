...
public class User {
	...
	public RepoCopy getWorkingCopy(String repoName) {
		...
		while(itr.hasNext()){
			RepoCopy tmp = itr.next();
			if(tmp.getReponame().equals(repoName)){
				return tmp;
			}
		}
		return null;
	...
	public void addToPendingCheckIn(Document doc, Change.Type type, String repoName) {
		...
		if(doc == null || type == null || repoName == null) 
			throw new IllegalArgumentException();
		Iterator<ChangeSet> itr = pendingCheckIns.iterator();
		boolean test = false;
		while (itr.hasNext()){
			ChangeSet tmp = itr.next();
			if(tmp.getReponame().equals(repoName)){
				tmp.addChange(doc, type);
				test = true;
			}
		}
		if(!test){
			ChangeSet set = new ChangeSet(repoName);
			set.addChange(doc, type);
		}
	...
	public ErrorType checkIn(String repoName) {
		...
		while(itr.hasNext()){
			...
			if(tmp.getReponame().equals(repoName)){
				...
				repo.queueCheckIn(tmp);
				pendingCheckIns.remove(tmp);
	...
	public ErrorType checkOut (String repoName) {
		...
		while(itr.hasNext()){
			RepoCopy tmp = itr.next();
			if(tmp.getReponame().equals(repoName)){
				workingCopies.remove(tmp);
			}
		}
		System.out.println("1.finished");
		// 2)
		Iterator<ChangeSet> itr2 = pendingCheckIns.iterator();
		while(itr2.hasNext()){
			ChangeSet tmp2 = itr2.next();
			if(tmp2.getReponame().equals(repoName)){
				pendingCheckIns.remove(tmp2);
			}
		}
		...
		if(isSubRepo(repoName)){
				Repo repo = VersionControlDb.findRepo(repoName);
				RepoCopy copy = new RepoCopy(repoName, repo.getVersion(),repo.getDocuments());
